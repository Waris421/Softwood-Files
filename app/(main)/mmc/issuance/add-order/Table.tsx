        ...
        if (!isLoading && initialData) {
            reset({ 
                items: initialData.map(item => ({ 
                    ...item,  
                    selected: false,
                    Balance: Math.min((1.02 * item.Required) - item.Issued, item.Received - item.Issued),
                })) 
            });
        }
            ...
            <table className="table w-full">
                <thead className={THEME.Table.HeaderRow}>
                    <tr className="bg-base-200">
                        ...
                        <th className="p-1 border-b text-center w-25">Required</th>
                        ...
                        <th className="p-1 border-b text-center w-25">Balance</th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        return (
                            ...
                            >
                                ...
                                <td className="p-1 w-100 text-center">
                                    <div className={THEME.TextInputReadOnly}>{field.Inventory} - {field.InventoryName}</div>
                                </td>
                                ...
                                <td className="p-1 w-25 text-center">
                                    <input 
                                        {...register(`items.${index}.Balance` as const, { valueAsNumber: true })} 
                                        className={`${THEME.TextInput} text-center`}
                                        type="number"
                                        min={0}
                                    />
                                    {errors.items?.[index]?.Balance && (
                                        <p className="text-[10px] text-red-500 mt-1">{errors.items[index]?.Balance?.message}</p>
                                    )}
                                </td>                
                            </tr>
                        )
