...
const TYPE_OPTIONS: DropdownOption[] = [
    ...
    {label: 'Before Wash', value: 'BW'},
    {label: 'After Wash', value: 'AW'},
]
...
export default function Filters({
    ...
    const [formData, setFormData] = useState({
        WorkOrder: 0, Type: 'Fab', Department: '', Inventories: [],
    });
    ...
    return (
        <div className="sticky top-16 z-30 bg-gray-100 dark:bg-gray-600 opacity-90 border-b border-base-200 px-4">
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-6">
                <FormField label='Work Order' error={errors.WorkOrder} required>
                    <SingleDropdownAsync
                        inputName="WorkOrder"
                        apiUrl={WORK_ORDER_OPTIONS_URL}
                        widthClass="w-full"
                        onSelect={(val: any) => handleInputChange('WorkOrder', val?.value)}
                    />
                </FormField>
