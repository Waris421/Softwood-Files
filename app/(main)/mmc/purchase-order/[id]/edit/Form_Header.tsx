...
export default function HeaderForm({ children }: { children?: React.ReactNode }) {
    ...
    // In edit mode: getCombinedData()['header'] has PO data from Django — pre-fills all fields
    useEffect(() => {
    ...
    return (
        <div className="flex flex-wrap items-start gap-4 p-4">
            {/* PO Number — read-only, comes from the URL */}
            <div className="flex flex-col gap-1">
                <label className="text-xs text-muted-foreground">PO Number</label>
                <div className={`${THEME.TextInput} w-24 text-center font-semibold`}>
                    #{id}
                </div>
            </div>
            {/* Supplier */}
