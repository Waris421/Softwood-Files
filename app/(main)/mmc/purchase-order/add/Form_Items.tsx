...
// This key identifies this form's slot in shared storage — FormContext uses it to store/retrieve inventory data
...
export default function ItemsForm() {
    ...
    // In add mode: getCombinedData()['inventory'] is undefined so reset() never runs — table stays at one blank row
    useEffect(() => {
    ...
    // Fires whenever any row changes — keeps shared storage always in sync with the table
    useEffect(() => {
    ...
    // If last row is deleted, append a fresh empty row so the table is never fully empty
