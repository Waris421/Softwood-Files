...
import { CATEGORY_OPTIONS, LEVEL_OPTIONS, MACHINE_TYPE_OPTIONS, SAM_FACTOR, SECTION_OPTIONS } from "./Constants";
...
const API_URL = (id: number) => `/api/productivity/operation/${id}/update`;

export function EditOperationModal({ isOpen, onClose, onSuccess, operationId }: EditOperationModalProps) {
    ...
    useEffect(() => {
        const fetchOperationData = async() => {
            ...
            setFetching(true);
            try {
                const response = await fetch(API_URL(operationId));
            ...
            <DialogContent className="bg-gray-100 dark:bg-gray-700 rounded-xl shadow-xl max-w-2xl mx-auto border border-base-200">
                <DialogHeader>
                    <DialogTitle className="text-xl font-bold text-base-content text-left">
                        Updating Operation # {operationId}: {formData.OperationName}
                    </DialogTitle>
