            ...
            >
                ...
                <div className="grid grid-cols-1 p-2">
                    {actions.map((action, index) => {
                        const className = "flex items-center gap-3 w-full p-3 rounded-md hover:bg-accent hover:text-accent-foreground transition-colors text-left";
                        const content = (
                            <>
                                {action.icon && (
                        ...
                        if (action.href) {
                            return (
                                <Link key={index} href={action.href} onClick={onClose} className={className}>
                                    {content}
                                </Link>
                            );
                        }
                        ...
                        return (
                            <button key={index} onClick={() => { action.onClick?.(); onClose(); }} className={className}>
                                {content}
                            </button>
                        );
                    })}
